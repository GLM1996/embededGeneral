...
export default function NewAppontment({ data, lastAppt, appointmentMongo }) {
  ...
  useEffect(() => {
    const initialize = async () => {
      if (data !== "Data" && data) {
        ...
        //setEndDate(formatDateTime(data.end).date);
        setStartTime(formatDateTime(data.start).time);
        //setEndTime(formatDateTime(data.end).time);
        ...
        if (itemMongo) {
          ...
          handleRealtorOrLender(itemMongo?.realtorLender);
  ...
  useEffect(() => {
            ...
            if (dataLender?.data?.[0]?.choices) {
              ...
              if (typeCita === "1ST APPT") {
                ...
                try {
                  ...
                  const filterResult = result.data.filter(item => item.typeAppt === "1ST APPT")

                  if (filterResult.length > 0) {
                    //REORDENAR LOS LENDERS SEGUN CANT DE APPT
                    // 1. Contar frecuencia
                    const frecuencia = filterResult.reduce((acc, obj) => {
                      acc[obj.realtorLenderValue] = (acc[obj.realtorLenderValue] || 0) + 1;
                      return acc;
                    }, {});

                    // 2. Filtrar actualLenders removiendo los que aparecen más de 3 veces
                    const filtrados = actualLenders.filter(lender => {
                      return (frecuencia[lender] || 0) < 6;
                    });

                    setDataRealtorLender(filtrados);
                    setDataRealtorLenderCopy(dataLender.data[0].choices || [])
                  } else {
    ...
    fetchData();
  }, [realtorLender]); // 👈 usamos debouncedMonto
  ...
  useEffect(() => {
    if (!startTime && data !== "Data") {
      setStartDate("")
      setStartTime("")
    }
  }, [realtorLenderValue, whereCita])
  ...
  const veryfiCalendarFreeSlots = async () => {
    ...
    const endDateTime = moment.tz(`${endDate}T23:59:59.999`, 'America/Los_Angeles').valueOf();
        ...
        try {
          ...
          if (result?.length > 0) {
            ...
            const nombres = filtered.map((cal) => {
              ...
              // 3. Agregar el sufijo condicionalmente
              if (realtorLender === 'LENDER') {
  ...
  const handleSave = async () => {
    ...
    if (data === 'Data') {
      ...
      try {
        const url = leadType !== "Personal Rent" ? `${servidor_n8n}/webhook/9f727616-ded2-4a94-a9dc-7ddb21eb5fe8` : `${servidor_n8n}/webhook-test/47c754ee-e28f-4206-92ce-348fe0ccd35d`
        const options = {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (
        ...
        realtorLenderValue !== itemMongo?.realtorLenderValue
      ) {
        const urlNew =
              ...
              <>
                ...
                <div className="col-12 ">
                  ...
                  <select
                    ...
                    required
                    disabled={statusSearchSlots || data !== "Data" || (realtorLenderValue && startDate && startTime)}
                  >
                    <option value="">EMPTY</option>
                    <option disabled>----------</option>
                    ...
                    <div className="col-12 d-flex gap-1 mt-1">
                      <div className="input-group input-group-sm">
                        <input
                          ...
                          required
                          disabled={statusSearchSlots || (realtorLenderValue && startDate && startTime) || !whereCita}
                        />
                      </div>
                      <div className="input-group input-group-sm">
                        <select
                          ...
                          required
                          disabled={statusSearchSlots || (realtorLenderValue && startDate && startTime) || !whereCita || !startDate}
                        >
                          <option value="">EMPTY</option>
                          <option disabled>----------</option>
