...
export default function ReasonDidSign({ handleFormData, personFilter, typeProblems, cleanProblems, cleanSelectProblems, problem }) {
        ...
        if (person && problem) {
            ...
            if (problem?.length > 0) {
                for (let i = 0; i < problem.length; i++) {
                    ...
                    if (person?.[customName] === "Yes" && !customLabel.includes("Other")) {
                        problemsData[customName] = "Yes"
                        optionsData[customLabel?.split('- ')?.[1]] = true;
                    } else {
                        problemsData[customName] = "No"
                        optionsData[customLabel?.split('- ')?.[1]] = false;
                    }

                    if (customLabel.includes("Other") && person?.[customName] !== "No" && person?.[customName]) {

                        problemsData["customNEWDidnTSignReasonOther"] = "Yes"
                        optionsData["Other"] = true;

                        setShowText(true);

                        if (person?.customNEWDidnTSignReasonOther) {
                            onChangeText(person?.customNEWDidnTSignReasonOther)
                        }
                    }
                }
            }

            const updated = { ...initialData, ...problemsData, textValue: person?.customNEWDidnTSignReasonOther };
            setFormData(updated);
    ...
    useEffect(() => {
        if (typeProblems !== "" && !loading && problem) {
            ...
            const sortedProblems = [...problem].sort((a, b) => {
                const aIsOther = a.label?.trim().toLowerCase() === "other";
                const bIsOther = b.label?.trim().toLowerCase() === "other";

                // "other" siempre al final
                if (aIsOther && !bIsOther) return 1;
                if (!aIsOther && bIsOther) return -1;

                // resto orden normal alfabético
                return (a.label || "").localeCompare(b.label || "", "es", { sensitivity: "base" });
            });

            setFilterProblems(sortedProblems);
    ...
    const handleCheckboxChange = (name, clave, status) => {
        ...
        if (clave !== "customNEWDidnTSignReasonOther") {
